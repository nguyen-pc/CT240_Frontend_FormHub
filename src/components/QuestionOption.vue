        ...
        <Popover>
            <PopoverTrigger as-child>
                <Button variant="outline" class="w-full justify-between">
                    {{ selectedLabel }}
                    <span class="ml-2 opacity-50">▼</span> <!-- Thay thế ChevronDown -->
                </Button>
            </PopoverTrigger>
            <PopoverContent class="w-[250px] p-2">
                <Command>
                    ...
                    <CommandList>
                        <CommandGroup>
                            <CommandItem v-for="option in options" :key="option.value" @select="selectOption(option)">
                                <span class="mr-2">{{ option.icon }}</span> <!-- Dùng emoji -->
                                {{ option.label }}
                            </CommandItem>
                        </CommandGroup>
                    </CommandList>
...
const selectedLabel = computed(() => {
    return options.find(option => option.value === selected.value)?.label || "Chọn loại câu hỏi";
});
